...
const BookingModal = ({ date, bookTicket, setBookTicket }) => {
    const { _id, name, time } = bookTicket;
        ...
        <div>
            ...
            <div class="modal modal-bottom sm:modal-middle">
                <div class="modal-box">
                    ...
                    <h3 class="font-bold text-lg text-center text-green-500">Booking for: {name}</h3>
                    <form onSubmit={handleBooking} className='grid grid-cols-1 gap-3 justify-items-center mt-2'>
                        ...
                        <select name='time' class="select select-bordered w-full max-w-xs">
                            {
                                time.map(slot => <option value={slot}>{slot}</option>)
                            }
                        </select>
                        ...
                        <input type="submit" value="Submit" class="btn btn-outline w-full max-w-xs" />
                    </form>
                </div>
            </div>
        </div >
